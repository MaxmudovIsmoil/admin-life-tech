...
@section('script')
    ...
    <script>
        ...
        function create_payment_datatable(url, student_id, group_id)
        {
            ...
            var dtable = payment_table.DataTable();
            dtable.destroy();
        ...
        $(document).ready(function() {
            ...
            $(document).on('submit', '#js_pay_form_modal', function(e) {
                ...
                var url2 = '{{ route("studentPayment.student_payments_in_group") }}';
                console.log(url2)
                ...
                $.ajax({
                    ...
                    success: (response) => {
                        ...
                        if(response.success == false) {
                            console.log(11)
                            $(this).siblings('.js_msg').html(response.warn)
                        }

                        if(response.success) {
                            // location.reload()
                            create_payment_datatable(url2, response.student_id, response.group_id)
                            $(this).find("select option:selected").removeAttr("selected");
                            $(this).find('.paid').val('')
                            // $(this).find('.js_student_payment_month').val('')
                            // $(this).find('.payment_type').val('')
                            // $(this).find('.discount_type').val('')
                        }
                    },
                    error: (response) => {
                        console.log('error: ', response)
